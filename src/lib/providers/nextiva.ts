 ...
 * SMS send:   POST /api/v1/sms/messages
 *
 * Stored in DB:
 *   provider_api_key      = Nextiva API key
...
const BASE = 'https://api.nextiva.com/api/v1'
...
async function apiFetch(path: string, apiKey: string, init: RequestInit = {}): Promise<unknown> {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Nextiva API ${res.status} ${path}: ${text}`)
  }
...
export const nextiva: Provider = {
  async registerWebhook(
    creds: ProviderCredentials,
    webhookBaseUrl: string,
  ): Promise<RegisterResult> {
    const apiKey = creds.apiKey!
    const url = `${webhookBaseUrl}/api/webhooks/nextiva`
  ...
  async sendSMS(
    ...
    await apiFetch('/sms/messages', creds.apiKey!, {
      method: 'POST',
